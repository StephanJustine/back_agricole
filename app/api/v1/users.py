...
async def create_user(
    ...
    Les utilisateurs créés par un admin sont automatiquement vérifiés.
    """
    # Vérification email
    if db.query(User).filter(User.email == request.email).first():
        raise HTTPException(
    ...
    # Vérification username (si fourni)
    ...
    user = User(
        ...
        nom=request.nom,
        prenom=request.prenom,
        telephone=request.telephone if hasattr(request, 'telephone') else None,
        hashed_password=SecurityService.get_password_hash(request.password),
        is_active=True,
        is_verified=True,  # <-- MODIFIÉ: Les admins créent des utilisateurs vérifiés
        role=request.role if hasattr(request, 'role') else UserRole.PRODUCTEUR  # <-- Utiliser le rôle du Producteur
    )
