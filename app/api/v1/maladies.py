...
"""
from fastapi import APIRouter, Depends, Query, HTTPException, status, BackgroundTasks,  Body
...
# ========================
# CRÉER UNE MALADIE BULK
# ========================

@router.post("/bulk", status_code=status.HTTP_201_CREATED)
async def create_maladies_bulk(
    requests: List[MaladieCreate] = Body(...),
    background_tasks: BackgroundTasks = None,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_active_user)
):
    """
    Création multiple de signalements de maladies (BULK).
    """
    ...
    for index, request in enumerate(requests):
        try:
            ...
            if culture.date_fin_reelle:
                errors.append({
                    "index": index,
                    "error": "Culture terminée"
                })
                continue

            # ========================
            # 2. Validation gravité
            # ========================
            if request.gravite is None or request.gravite < 1 or request.gravite > 5:
                errors.append({
                    "index": index,
                    "error": "Gravité doit être entre 1 et 5"
                })
                continue
            ...
            maladie = Maladie(
                ...
                gravite=request.gravite,
                surface_touchee=request.surface_touchee,
                traitement_applique=request.traitement_applique.strip() if request.traitement_applique else None,
                date_traitement=request.date_traitement,
                observations=request.observations.strip() if request.observations else None,
                recommandations=request.recommandations.strip() if request.recommandations else None,
                photo_url=request.photo_url
            )

            # ========================
            # 4. Recommandations auto
            # ========================
            if not maladie.recommandations:
                maladie.recommandations = get_recommandation_auto(
                    maladie.type,
                    maladie.gravite
                )
            ...
            # 6. Alerte critique
            # ========================
            if getattr(maladie, "est_critique", False):
                background_tasks.add_task(
                    send_alerte_maladie,
                    maladie_id=maladie.id,
                    culture=culture,
                ...
                if hasattr(maladie, "marquer_alerte_envoyee"):
                    maladie.marquer_alerte_envoyee()
                    db.commit()
